...
class TestGraphState:
    ...
    async def test_basic_state_access(self):
        ...
        @node(start=True)
        async def node_counter_init(input: str, graph_state: dict):
            """Initialize the counter in the graph state."""
            # Initialize a counter in the graph state
            graph_state["counter"] = 1
            return {"message": "Counter initialized"}
        
        @node
        async def node_counter_increment(input: str, graph_state: dict):
        ...
        @node
        async def node_counter_double(input: str, graph_state: dict):
            """Double the counter in the graph state."""
            # Double the counter
            graph_state["counter"] *= 2
            return {"message": "Counter doubled"}
        
        @node(end=True)
        async def node_counter_result(input: str, graph_state: dict):
            """Return the final counter value."""
            # Return the counter value
            return {"result": graph_state["counter"], "message": "Final counter"}
        
        # Create a graph with initial state
        graph = Graph(initial_state={"initialized": True})
        
        # Add nodes
        nodes = [node_counter_init, node_counter_increment, 
                 node_counter_double, node_counter_result]
        ...
        assert graph.get_state()["initialized"] == True
    ...
    async def test_initial_state(self):
        ...
        async def node_check_state(input: str, graph_state: dict):
            """Check that the initial state is accessible."""
            return {"initial_value": graph_state["initial_value"]}
    ...
    async def test_state_persistence_between_executions(self):
        ...
        @node(start=True, end=True)
        async def node_increment_counter(input: str, graph_state: dict):
            """Increment a counter in the graph state."""
            if "counter" not in graph_state:
                graph_state["counter"] = 0
            graph_state["counter"] += 1
            return {"counter": graph_state["counter"]}
    ...
    async def test_update_state_api(self):
        ...
        @node(start=True, end=True)
        async def node_read_state(input: str, graph_state: dict):
            """Read values from the graph state."""
            return {
                "value1": graph_state.get("value1", None),
                "value2": graph_state.get("value2", None)
            }
    ...
    async def test_complex_state_manipulation(self):
        ...
        @node(start=True)
        async def node_init_complex_state(input: str, graph_state: dict):
        ...
        @node
        async def node_modify_state(input: str, graph_state: dict):
            """Modify the complex state."""
            # Add a new user
            graph_state["users"]["charlie"] = {"age": 35, "roles": ["user", "reviewer"]}
            # Modify an existing user
            graph_state["users"]["alice"]["roles"].append("owner")
            return {"message": "State modified"}
        
        @node(end=True)
        async def node_summarize_state(input: str, graph_state: dict):
            """Summarize the state."""
            users = graph_state["users"]
            return {
                "user_count": len(users),
                "admin_users": [name for name, data in users.items() 
